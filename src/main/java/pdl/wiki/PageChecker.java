...
public abstract class PageChecker {
	 ...
	 * @return -1 si le lien fourni n'est pas exploitable ou le nombre de tableaux
	 *         présents sur la page (0 ou +)
	 */
	...
	/**
	 * @param pUrl
	 * @return true si la page est un lien existant de Wikipédia, false si la page
	 *         n'appartient pas au domaine Wikipédia ou est injoignable
	 */
	private static boolean pageExist(String pUrl) {
		// On vérifie d'abord que le lien renvoi vers une page existante
		Connection.Response response = null;
		try {
			// response = Jsoup.connect(pUrl).ignoreHttpErrors(true).execute();
			response = Jsoup.connect(pUrl).execute();
			if (response.statusCode() == 200) {
				// Si la librairie a réussi à se connecter on teste ensuite si la page provient
				// de wikipedia
	...
	/**
	 * La méthode est appelée uniquement si le lien a passé la 1ère vérification et
	 * compte le nombre de tableaux existants sur la page
	 *
	 * @param pUrl
	...
	private static List<Element> pageContainsTable(String pUrl) {
		...
		try {
			...
			// Récupération des tableaux de Wikipedia via le selecteur css sur la classe
			// wikitable propre aux tableaux
